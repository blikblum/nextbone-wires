...
export default class BooksView extends Component {
  render() {
    html`<div class="container">
      ...
      <div class="row"><div class="flip-scene col-md-9 order-1 order-md-2">
    <div class="books__viewer flip-card"/>
      <div class="list-group">
        ${this.collection.map(book => {
          const {title, year, author, id} = book.attributes
          return html` <a class="list-group-item list-group-item-action flex-column" route="books.show" param-bookid=${id}>
            <h5 class="mb-1">${title} <small>(${year})</small></h5><p class="mb-1">${author}</p>            
          </a>`;
        })}
      </div>
    </div>
